...
public static partial class GameObjectExtensions {
	...
	/// <summary>
	///     Gets the first child GameObject with the specified name. If there is no GameObject with the speficided name,
	///     returns null.
	/// </summary>
	...
	public static ChildrenEnumerable Children(this GameObject origin) => new(origin, false);

	/// <summary>Returns a collection of GameObjects that contain this GameObject, and the child GameObjects.</summary>
	public static ChildrenEnumerable ChildrenAndSelf(this GameObject origin) => new(origin, true);

	/// <summary>Returns a collection of the ancestor GameObjects of this GameObject.</summary>
	public static AncestorsEnumerable Ancestors(this GameObject origin) => new(origin, false);

	/// <summary>Returns a collection of GameObjects that contain this element, and the ancestors of this GameObject.</summary>
	public static AncestorsEnumerable AncestorsAndSelf(this GameObject origin) => new(origin, true);

	/// <summary>Returns a collection of the descendant GameObjects.</summary>
	public static DescendantsEnumerable Descendants(this GameObject origin, Func<Transform, bool> descendIntoChildren = null) => new(origin, false, descendIntoChildren);

	/// <summary>
	///     Returns a collection of GameObjects that contain this GameObject, and all descendant GameObjects of this
	///     GameObject.
	/// </summary>
	public static DescendantsEnumerable DescendantsAndSelf(this GameObject origin, Func<Transform, bool> descendIntoChildren = null) => new(origin, true, descendIntoChildren);

	/// <summary>Returns a collection of the sibling GameObjects before this GameObject.</summary>
	public static BeforeSelfEnumerable BeforeSelf(this GameObject origin) => new(origin, false);

	/// <summary>
	///     Returns a collection of GameObjects that contain this GameObject, and the sibling GameObjects before this
	///     GameObject.
	/// </summary>
	public static BeforeSelfEnumerable BeforeSelfAndSelf(this GameObject origin) => new(origin, true);

	/// <summary>Returns a collection of the sibling GameObjects after this GameObject.</summary>
	public static AfterSelfEnumerable AfterSelf(this GameObject origin) => new(origin, false);

	/// <summary>
	///     Returns a collection of GameObjects that contain this GameObject, and the sibling GameObjects after this
	///     GameObject.
	/// </summary>
	public static AfterSelfEnumerable AfterSelfAndSelf(this GameObject origin) => new(origin, true);
	...
	public struct ChildrenEnumerable : IEnumerable<GameObject> {
		...
		public void Destroy(bool useDestroyImmediate = false, bool detachParent = false) {
			var e = GetEnumerator();
			while (e.MoveNext()) e.Current.Destroy(useDestroyImmediate);
		...
		public void Destroy(Func<GameObject, bool> predicate, bool useDestroyImmediate = false) {
			...
			while (e.MoveNext()) {
				var item = e.Current;
				if (predicate(item)) item.Destroy(useDestroyImmediate);
			}
		}
...
#region LINQ
		...
		public GameObject[] ToArray() {
			...
			if (array.Length != len) Array.Resize(ref array, len);
		...
		public struct OfComponentEnumerable<T> : IEnumerable<T>
			...
			public OfComponentEnumerator<T> GetEnumerator() => new(ref parent);
...
#if UNITY_EDITOR
			private static readonly List<T> componentCache = new(); // for no allocate on UNITY_EDITOR
#endif
...
#region LINQ
	...
	public struct DescendantsEnumerable : IEnumerable<GameObject> {
		...
		public Enumerator GetEnumerator() {
			// check GameObject is destroyed only on GetEnumerator timing
			if (origin == null) return new Enumerator(null, withSelf, false, null, descendIntoChildren);
...
#region LINQ
		...
		private void DescendantsCore<TState, T>(ref Func<GameObject, TState> let, ref Func<TState, bool> filter, ref Func<TState, T> selector, ref Transform transform,
			ref int index, ref T[] array) {
			if (!descendIntoChildren(transform)) return;
		...
		internal class InternalUnsafeRefStack {
			...
			public static Queue<InternalUnsafeRefStack> RefStackPool = new();
			...
			public void Push(ref Enumerator e) {
				if (size == array.Length) Array.Resize(ref array, array.Length * 2);
		...
		public struct Enumerator : IEnumerator<GameObject> {
			...
			private int currentIndex;
			private GameObject current;
			private readonly InternalUnsafeRefStack sharedStack;
			private readonly Func<Transform, bool> descendIntoChildren;
			...
			public bool MoveNext() {
				...
				if (MoveNextCore(false, out current)) return true;
				// reuse
				canRun = false;
				InternalUnsafeRefStack.RefStackPool.Enqueue(sharedStack);
				return false;
		...
		public struct Enumerator : IEnumerator<GameObject> {
			...
			public bool MoveNext() {
				...
				if (currentIndex < childCount) {
					...
					if (item == originTransform) goto RETURN_SELF;
