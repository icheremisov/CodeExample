...
namespace XLib.UI.Controls.LoopScroll {
	...
	[AddComponentMenu("UI/Loop Vertical Scroll Rect", 51), DisallowMultipleComponent]
	public class UILoopVerticalScrollRect : UILoopScrollRect {
		...
		protected override bool UpdateItems(Bounds viewBounds, Bounds contentBounds) {
			...
			if (viewBounds.max.y < contentBounds.min.y && itemTypeEnd > itemTypeStart) {
				...
				if (totalCount >= 0) maxItemTypeStart = Mathf.Max(0, totalCount - (itemTypeEnd - itemTypeStart));
				...
				var offsetCount = Mathf.FloorToInt((contentBounds.min.y - viewBounds.max.y) / (elementSize + contentSpacing));
			...
			if (viewBounds.min.y > contentBounds.max.y && itemTypeEnd > itemTypeStart) {
				...
				var offsetCount = Mathf.FloorToInt((viewBounds.min.y - contentBounds.max.y) / (elementSize + contentSpacing));
