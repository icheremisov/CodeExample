...
namespace RectEx {
    public static class MiscellaniousExtensions {
        ...
        public static Rect SplitHorizontal(this Rect rect, int index, int total){
            rect = rect.Abs();
            return new Rect(
                x: rect.x + rect.width * index / total,
                y: rect.y,
                width: rect.width / total,
                height: rect.height
            );
        }
        public static Rect SplitVertical(this Rect rect, int index, int total){
            rect = rect.Abs();
            return new Rect(
                x: rect.x,
                y: rect.y + rect.height * index / total,
                width: rect.width,
